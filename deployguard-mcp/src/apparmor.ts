...
// buildProfile() stamps its own recipe into the header comment ("type: X,
// level: Y, runtime: Z") — the only record of what a profile was actually
// generated with. Parsed back out so a reapply against an app that's
// already deployed can reuse it instead of trusting whatever the caller
// passes this time, which is exactly how a real redeploy silently
// regressed a working profile down to runtime "none" (dropping the node-
// specific rules) when the calling agent passed different parameters than
// the original deploy used.
function parseProfileHeader(content: string): { type: ResourceType; level: SecurityLevel; runtime: Runtime } | null {
  const match = content.match(/^# Generated by DeployGuard for .+ \(type: (\S+), level: (\S+), runtime: (\S+)\)\.$/m);
  if (!match) return null;
  const [, type, level, runtime] = match;
  if (type !== "web-service") return null;
  if (!["low", "medium", "high"].includes(level)) return null;
  if (!["node", "python", "cpp", "none"].includes(runtime)) return null;
  return { type: type as ResourceType, level: level as SecurityLevel, runtime: runtime as Runtime };
}
...
//
// A profile already installed at this path means this is a redeploy of an
// app DeployGuard already manages, not a first deploy — in that case the
// existing profile's own recorded {type, level, runtime} always wins over
// whatever this call was given, so a routine reapply can never silently
// drift the security configuration. If the existing file's header can't be
// read back (missing, or not one DeployGuard generated), this refuses to
// guess — it fails rather than quietly overwriting an unknown file with a
// fresh profile built from this call's parameters.
export async function applyProfile(
  ...
  if (existing !== null) {
    const recorded = parseProfileHeader(existing);
    if (!recorded) {
      return {
        status: "fail",
        summary:
          `FAIL: a profile already exists at ${file}, but its configuration couldn't be determined (missing or ` +
          "unrecognized header) — refusing to silently overwrite it with a freshly built profile. Remove it " +
          "manually first if you want to regenerate from scratch, or restore its original DeployGuard-generated " +
          "header comment if it was hand-edited.",
        mode,
      };
    }
    if (recorded.type !== type || recorded.level !== level || recorded.runtime !== runtime) {
      reusedNote =
        ` Note: this app was already deployed with type=${recorded.type}, level=${recorded.level}, ` +
        `runtime=${recorded.runtime} — reusing that existing configuration instead of the ` +
        `type=${type}, level=${level}, runtime=${runtime} passed this time, so a routine reapply can't ` +
        "silently drift the security configuration. To actually change level/runtime for this app, remove " +
        "the existing profile first, then generate/apply a new one deliberately.";
    }
    type = recorded.type;
  ...
  return {
    status: "ok",
    summary:
      `OK: loaded ${file} in enforce mode. Bind it to a process via a systemd unit's AppArmorProfile= ` +
      `to actually confine it (if not already bound).${reusedNote}`,
    mode,
  };
}
  ...
  server.registerTool(
    "apparmor.apply_profile",
    {
      description:
        ...
        "— that needs a systemd unit to reference it. REDEPLOY SAFETY: if a profile already exists for this " +
        "path, its own previously-recorded type/level/runtime always wins over whatever this call passes — a " +
        "routine reapply (e.g. switching complain to enforce) can never silently regress an app's security " +
        "configuration just because different parameters were passed this time. To deliberately change an " +
        "app's level or runtime, remove its existing profile file first, then apply a new one on purpose.",
      inputSchema: {
        path: z.string(),
        type: z.enum(["web-service"]),
        level: z.enum(["low", "medium", "high"]),
